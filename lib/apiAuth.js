...
// Verifies the calling API key's owner actually owns business_id before
// letting it attach a promo code/offer/etc. to that business. Without this,
// any write-scoped key could attach content to someone else's listing since
// these routes run through the service-role client and bypass RLS entirely.
export async function assertOwnsBusiness(admin, keyRow, businessId) {
  if (hasScope(keyRow, 'admin')) return null;
  const { data: business } = await admin.from('businesses').select('owner_id').eq('id', businessId).maybeSingle();
  if (!business) return Response.json({ error: 'business_id not found' }, { status: 404 });
  if (business.owner_id !== keyRow.owner_id) return Response.json({ error: 'You do not own this business.' }, { status: 403 });
  return null;
}
