...
export function getBrainConfig() {
  ...
  cachedConfig = {
    ...
    providerIgnore,
    timeoutMs: 30_000,
    maxAttempts: 4, // free pool: 429s, empty/degenerate and PII-redacted replies are all transient per-model - budget enough tries to land a clean one
    retryDelayMs: 900,
    temperature: 0.7,
    topP: 0.9,
    maxTokens: 600
...
function looksDegenerate(reply) {
  ...
  // Compound markers too: live traffic produced "[PERSON_NAME]" (a yacht
  // name redacted as a person), so match any underscore-joined variant of
  // the known PII tokens, e.g. [FULL_NAME], [PERSON_NAME], [STREET_ADDRESS].
  if (/\[(?:[A-Z]{2,}_)*(?:ADDRESS|LOCATION|EMAIL|PHONE|NAME|PERSON|CITY)(?:_[A-Z]{2,})*\]/.test(r)) return true;
  return false;
}
