...
export async function dispatchWebhookEvent(eventType, payload) {
  ...
  await Promise.all(matching.map(async (hook) => {
    ...
    let status = 'failed';
    if (validateWebhookUrl(hook.url)) {
      await admin.from('webhook_deliveries').insert({ webhook_id: hook.id, event_type: eventType, payload, response_status: null, status: 'failed' });
      return;
    }
    try {
