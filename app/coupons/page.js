...
export default async function CouponsPage() {
  const [allDeals, { t }] = await Promise.all([getAllDeals(), getServerTranslations()]);

  return (
    <main className="flex-1 py-10">
      <div className="market-shell mb-8 rounded-[2rem] p-8">
        <p className="text-sm uppercase tracking-[0.3em] text-brand-gold">{t.coupons.title}</p>
        <h1 className="mt-2 text-3xl font-semibold text-white">{t.coupons.subtitle}</h1>
      </div>
