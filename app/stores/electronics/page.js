...
export default async function ElectronicsCategoryPage() {
  const deals = await getDealsByCategory('electronics');
  const { t } = await getServerTranslations();

  return (
    <main className="flex-1 py-10">
      <div className="mb-8 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-premium">
        <p className="text-sm uppercase tracking-[0.3em] text-brand-gold">{t.categories.electronics}</p>
        <h1 className="mt-2 text-3xl font-semibold text-white">{t.storesCategoryTagline.electronics}</h1>
      </div>
