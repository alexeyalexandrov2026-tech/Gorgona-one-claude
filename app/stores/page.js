  ...
  return (
    <main className="flex-1 py-10">
      <div className="market-shell mb-8 rounded-[2rem] p-8">
        <p className="text-sm uppercase tracking-[0.3em] text-brand-gold">{t.stores.directoryTitle}</p>
        <h1 className="mt-2 text-3xl font-semibold text-white">{t.stores.directoryHeading}</h1>
      </div>
      ...
      <div className="mb-8 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {categories.map((category) => (
          <Link key={category.slug} href={`/stores/${category.slug}`} className="market-card rounded-2xl p-4">
            ...
            <p className="mt-2 font-semibold text-white">{t.categories[category.slug] || category.label}</p>
